...
The AI workforce is an OMO (Oh My OpenAgent) fleet implemented as the `hermes-omo-plugin`, which launches named subagents natively through Hermes' subagent lifecycle — no OpenCode server, no `opencode` tool, no `omo.jsonc`. The roster (agent → model → fallback chains, category routing, and the Claude escalation policy) is defined in the `omo:` values block and rendered into the `openagent-omo-config` ConfigMap (`templates/omo-config.yaml`). Model IDs live only in `litellm.proxy_config.model_list`; a model appears exactly once. Chains use the `omo.chains` array (primary-first); runtime fallback on 429/5xx is owned by the plugin's `runtime_fallback` state machine.
...
| hermes-agent.config.plugins.entries.omo.settings.cooldown_seconds | int | `30` |  |
| hermes-agent.config.plugins.entries.omo.settings.max_fallback_attempts | int | `3` |  |
| hermes-agent.config.plugins.entries.omo.settings.mcp_enabled | bool | `true` |  |
| hermes-agent.config.plugins.entries.omo.settings.restore_primary_after_cooldown | bool | `true` |  |
